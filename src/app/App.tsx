...
function AcidEquilibriumSimulator() {
  ...
  const rangeClassName =
    "w-full h-3 rounded-full appearance-none cursor-pointer border border-border shadow-inner [&::-webkit-slider-thumb]:appearance-none [&::-webkit-slider-thumb]:h-5 [&::-webkit-slider-thumb]:w-5 [&::-webkit-slider-thumb]:rounded-full [&::-webkit-slider-thumb]:border-2 [&::-webkit-slider-thumb]:border-white [&::-webkit-slider-thumb]:bg-[#0D1B2A] [&::-webkit-slider-thumb]:shadow-md [&::-moz-range-thumb]:h-5 [&::-moz-range-thumb]:w-5 [&::-moz-range-thumb]:rounded-full [&::-moz-range-thumb]:border-2 [&::-moz-range-thumb]:border-white [&::-moz-range-thumb]:bg-[#0D1B2A] [&::-moz-range-thumb]:shadow-md";
      ...
      <div className="space-y-4">
        <div className="rounded-lg border border-border bg-muted/30 p-3">
          <div className="flex items-center justify-between gap-3 text-xs mb-2">
            <span className="font-medium text-foreground">PaCO₂ (respiratory)</span>
            <span className="font-mono rounded-md bg-[#0D1B2A] px-2 py-1 text-[#4DD9E0]">
              {co2kPa.toFixed(1)} kPa
            </span>
          </div>
          <input
            ...
            style={{
              background:
                "linear-gradient(90deg, #1A9DA6 0%, #1A9DA6 27%, #27AE60 27%, #27AE60 40%, #E8534A 40%, #E8534A 100%)",
              accentColor: "#0A6E74",
            }}
          />
          <div className="relative h-3 mt-1">
            <div className="absolute top-0 h-3 rounded-sm border-x border-[#27AE60] bg-[#27AE60]/20 left-[27%] w-[13%]" />
          </div>
          <div className="flex justify-between text-[10px] text-muted-foreground mt-1">
            <span>2.0 kPa - alkalosis</span>
            <span className="text-[#27AE60] font-medium">Normal 4.7–6.0</span>
        ...
        <div className="rounded-lg border border-border bg-muted/30 p-3">
          <div className="flex items-center justify-between gap-3 text-xs mb-2">
            <span className="font-medium text-foreground">Ketone level (metabolic)</span>
            <span className="font-mono rounded-md bg-[#0D1B2A] px-2 py-1 text-[#F5A623]">
              {ketones.toFixed(1)} mmol/L
            </span>
          </div>
          <input
            ...
            style={{
              background:
                "linear-gradient(90deg, #27AE60 0%, #27AE60 37.5%, #F5A623 37.5%, #F5A623 75%, #E8534A 75%, #E8534A 100%)",
              accentColor: "#F5A623",
            }}
          />
          <div className="flex justify-between text-[10px] text-muted-foreground mt-1">
            <span>Absent</span>
            <span className="text-[#F5A623] font-medium">Mild (&lt;1.5)</span>
            <span className="text-[#E8534A] font-medium">Severe DKA (&gt;3)</span>
          </div>
        </div>
      </div>
...
function SystematicModule({ onReviewedAll }: { onReviewedAll: () => void }) {
  ...
  const handleStep = (i: number) => {
    const updatedDoneSteps = doneSteps.includes(i)
      ? doneSteps
      : [...doneSteps, i];
    ...
    if (updatedDoneSteps.length === steps.length) {
      onReviewedAll();
    }
  };
...
function ScenariosModule({ onComplete }: { onComplete: () => void }) {
  ...
  const [choiceOrders] = useState(() =>
    SCENARIOS.map((scenario) => shuffledIndices(scenario.choices.length))
  );
  const scenario = SCENARIOS[scenarioIdx];
  const choices = choiceOrders[scenarioIdx].map((choiceIdx) => scenario.choices[choiceIdx]);
...
function AssessmentModule({
  ...
  const [optionOrders] = useState(() =>
    QUIZ_QUESTIONS.map((question) => shuffledIndices(question.options.length))
  );

  const q = QUIZ_QUESTIONS[currentQ];
  const options = optionOrders[currentQ].map((optionIdx) => ({
    option: q.options[optionIdx],
    originalIdx: optionIdx,
  }));

  const handleAnswer = (i: number) => {
    if (answered) return;
    const { option, originalIdx } = options[i];
    setSelected(i);
    setAnswered(true);
    setScores((s) => [...s, option.correct]);
    setAnswers((a) => [...a, { questionIdx: currentQ, selectedIdx: originalIdx, correct: option.correct }]);
  };
  ...
  const handleNext = () => {
    ...
    if (next >= QUIZ_QUESTIONS.length) {
      ...
      const total = finalAnswers.filter((answer) => answer.correct).length;
      setFinished(true);
...
function CompletionScreen({
  ...
  const downloadCertificate = async () => {
    ...
    try {
      ...
      // Bottom row — Date | Status (matching on-screen layout)
      const rowY = 126;
      // Date
      pdf.setFont("helvetica", "bold");
      pdf.setFontSize(10);
      pdf.setTextColor(13, 27, 42);
      pdf.text(dateStr, cx - 34, rowY, { align: "center" });
      pdf.setFont("helvetica", "normal");
      pdf.setFontSize(7);
      pdf.setTextColor(90, 106, 122);
      pdf.text("Date of Completion", cx - 34, rowY + 5, { align: "center" });

      // Status
      pdf.setFont("helvetica", "bold");
      pdf.setFontSize(10);
      pdf.setTextColor(13, 27, 42);
      pdf.text("Passed", cx + 34, rowY, { align: "center" });
      pdf.setFont("helvetica", "normal");
      pdf.setFontSize(7);
      pdf.setTextColor(90, 106, 122);
      pdf.text("Status", cx + 34, rowY + 5, { align: "center" });
            ...
            {ratingSubmitted && (
              ...
              >
                ...
                <div className="flex flex-wrap justify-center gap-3">
                  ...
                  <button
                    onClick={onContinue}
                    className="flex items-center gap-2 border border-border bg-card text-foreground px-6 py-3 rounded-lg font-medium hover:bg-muted transition-colors"
                  >
                    Continue
    ...
    <div className="text-center space-y-5 py-8">
      ...
      <div className="space-y-2">
        ...
        <p className="text-sm text-muted-foreground max-w-md mx-auto">
          Thank you for completing the Blood Gas Interpretation Resource.
        </p>
...
export default function App() {
  ...
  const [systematicReviewed, setSystematicReviewed] = useState(false);
  ...
  const handleCertificateContinue = () => {
    ...
    if (currentModule < MODULES.length - 1) {
      ...
      saveProgress(updated, nextModule);
  ...
  const renderContent = () => {
    ...
    switch (mod.id) {
      ...
      case "systematic":
        return <SystematicModule onReviewedAll={() => setSystematicReviewed(true)} />;
      case "disorders":
  ...
  const showNextButton =
    !showCompletion &&
    mod.type === "content" &&
    (mod.id !== "systematic" || systematicReviewed) &&
